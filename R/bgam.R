...
update.scale.p.gourp <- function(prior="mde", df=1, b0, ss, theta, group.vars)
{
  if (prior == "mde"){
    den0 <- (2 * ss[1])^(-1) * exp(-abs(b0)/ss[1]) # de density
    den1 <- (2 * ss[2])^(-1) * exp(-abs(b0)/ss[2])
  ...
  for(j in 1:length(group.vars)){
    ...
    tmp_theta <- unqie(theta[vars])
    p[vars] <- tmp_theta * prod(den1[vars]) / (tmp_theta * prod(den1[vars]) + (1 - tmp_theta) * prod(den0[vars]) + 1e-10)
